...
instance Read PasswdEntry where
  readsPrec _ value =
    case split ':' value of
      ...
      x -> error $ "Invalid number fo fields in input: " ++ show x
    ...
    where
      ...
      split _ [] = [[]]
      split delim str =
        -- Find the part of the list before delim and put it in
...
inputToMaps input = (uidmap, usermap)
  ...
  where
    ...
    entries = map read (filter (notElem '#') . lines $ input)
