...
routes = do
  ...
  get "/auth/verifyEmail/:code" $ do
    ...
    case result of
      ...
      Left EmailVerificationErrorInvalidEmail ->
        renderHtml $ verifyEmailPage "The email is invalid"
...
verifyEmailPage msg = mainLayout "Email Verification" $ do
...
authFormLayout view formTitle action msgs = formLayout view action $ do
 ...
 where
  ...
  errorList = H.ul . concatMap errorItem
